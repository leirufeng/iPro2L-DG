...
if __name__ == '__main__':
    ...
    train_pos_seqs = np.array(read_fasta('../Liu_data/promoter.txt'))
    train_neg_seqs = np.array(read_fasta('../Liu_data/non-promoter.txt'))
    ...
    train_label = np.array([1] * 2860 + [0] * 2860).astype(np.float32)
    ...
    pd.DataFrame(np.array(ten_all_performance)).to_csv('../files/liu_10_cycle_result.csv',index=False)

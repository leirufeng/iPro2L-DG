...
def build_model(windows=5, weight_decay=1e-4):
    input_1 = Input(shape=(windows, 81))

    x = Conv1D(filters=96, kernel_size=3, strides=1, kernel_initializer="he_uniform", padding="same",
               use_bias=False)(input_1)

    transformer = Transformer(encoder_stack=4,
